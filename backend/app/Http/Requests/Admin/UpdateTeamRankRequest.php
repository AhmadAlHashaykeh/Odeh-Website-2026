...
class UpdateTeamRankRequest extends FormRequest
{
    ...
    {
        $id = $this->route('team_rank') ?? $this->route('id');

        return [
            'name' => ['sometimes', 'required', 'string', 'max:255'],
            'slug' => ['sometimes', 'nullable', 'string', 'max:255', Rule::unique('team_ranks', 'slug')->ignore($id)],
            'color' => ['sometimes', 'required', 'string', 'max:32', 'regex:/^#([A-Fa-f0-9]{6}|[A-Fa-f0-9]{3})$/'],
